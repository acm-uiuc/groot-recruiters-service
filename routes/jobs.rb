  ...
  module JobsRoutes
    def self.registered(app)
      ...
      app.post '/jobs' do
        ...
        status, error =
          Job.validate(params, %i[job_title organization contact_name contact_email contact_phone job_type description])
        halt status, ResponseFormat.error(error) if error

        Job.first_or_create(
          { title: params[:job_title], company: params[:organization] },
      ...
      app.put '/jobs/:job_id/approve' do
        ...
        status, error = Job.validate(params, %i[job_id])
        halt(status, ResponseFormat.error(error)) if error
